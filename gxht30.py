...
-------------- Description: ------------------
   FileName : gxht30.py
   Author : 绒毛宝贝
   ProjectName : MicroPython驱动gxht30温湿度IC
   IDE Version : PyCharm
   Date:2021/9/7 10:00
...
# 在GXHT30与SHT3x系列温度IC中 I2C 通讯地址通常有两个：Address A 0x44 \ Address B 0x45
# 此处使用地址A 即十六进制0x44。换算为十进制地址为68.
DEFAULT_I2C_ADDRESS = 0x44

class GXHT30():
    """
    GXHT30 sensor driver in pure micropython based on I2C bus
    
    References: 
    * https://www.sensirion.com/fileadmin/user_upload/customers/sensirion/Dokumente/2_Humidity_Sensors/Sensirion_Humidity_Sensors_SHT3x_Datasheet_digital.pdf
    * https://www.wemos.cc/sites/default/files/2016-11/GXHT30-DIS_datasheet.pdf
    ...
    * https://www.sensirion.com/fileadmin/user_upload/customers/sensirion/Dokumente/11_Sample_Codes_Software/Humidity_Sensors/Sensirion_Humidity_Sensors_SHT3x_Sample_Code_V2.pdf
    ...
    def __init__(self, scl_pin='B10', sda_pin='B9', delta_temp = 0, delta_hum = 0, i2c_address=DEFAULT_I2C_ADDRESS):
        self.i2c = SoftI2C(scl=Pin(scl_pin), sda=Pin(sda_pin))
        self.i2c_addr = i2c_address
        self.set_delta(delta_temp, delta_hum)
        time.sleep_ms(50)
    
    def init(self, scl_pin='B10', sda_pin='B9'):
        """
        ...
        try:
            ...
            for i in range(response_size//3):
                if not self._check_crc(data[i*3:(i+1)*3]): # pos 2 and 5 are CRC
                    raise GXHT30Error(GXHT30Error.CRC_ERROR)
            if data == bytearray(response_size):
                raise GXHT30Error(GXHT30Error.DATA_ERROR)
            return data
        except OSError as ex:
            if 'I2C' in ex.args[0]:
                raise GXHT30Error(GXHT30Error.BUS_ERROR)
            raise ex
    ...
    def clear_status(self):
        ...
        """
        return self.send_cmd(GXHT30.CLEAR_STATUS_CMD, None); 

    def reset(self):
        """
        Send a soft-reset to the sensor
        """
        return self.send_cmd(GXHT30.RESET_CMD, None); 
    ...
    def status(self, raw=False):
        ...
        """
        data = self.send_cmd(GXHT30.STATUS_CMD, 3, read_delay_ms=20); 
    ...
    def measure(self, raw=False):
        ...
        """
        data = self.send_cmd(GXHT30.MEASURE_CMD, 6); 
...
class GXHT30Error(Exception):
    ...
    def get_message(self):
        if self.error_code == GXHT30Error.BUS_ERROR:
            return "Bus error"
        elif self.error_code == GXHT30Error.DATA_ERROR:
            return "Data error"
        elif self.error_code == GXHT30Error.CRC_ERROR:
            return "CRC error"
        else:
